...
double ReadDetectorMassTons(const json& config)
{
  ...
  return RequiredDouble(detector, "crystal_count") *
         RequiredDouble(detector, "crystal_mass_kg") / 1000.0;
}
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    std::ofstream recoilByComponent(paths.recoilByComponentCsv);
    if (!recoilByComponent) {
      throw std::runtime_error("failed to open output '" + paths.recoilByComponentCsv + "'");
    }

    std::ofstream timeByComponent(paths.timeByComponentCsv);
    if (!timeByComponent) {
      throw std::runtime_error("failed to open output '" + paths.timeByComponentCsv + "'");
    }

    std::ofstream recoilTime(paths.recoilTimeCsv);
    if (!recoilTime) {
      throw std::runtime_error("failed to open output '" + paths.recoilTimeCsv + "'");
    }

    std::ofstream nuRecoil(paths.nuRecoilCsv);
    if (!nuRecoil) {
      throw std::runtime_error("failed to open output '" + paths.nuRecoilCsv + "'");
    }
    ...
    RateLedger ledger;
    std::map<double, std::map<std::string, double>> recoilEventsByComponent;
    std::map<double, std::map<std::string, std::map<double, double>>>
      nuRecoilEventsByComponent;
    for (const auto& isotope : isotopeList) {
    ...
    for (const auto& component : components) {
      ...
      for (double timeStart = timeMinNs; timeStart < timeMaxNs; timeStart += timeStepNs) {
        ...
        const double expectedEvents =
          ledger.expectedEventsByComponent[component.component] * probability;
    ...
    for (const auto& isotope : isotopeList) {
      isotopeLedger.push_back({
        ...
        {"expected_events", ledger.expectedEventsByIsotope[isotope.name]},
      });
    }
    ...
    for (const auto& component : components) {
      ...
      componentLedger.push_back({
        ...
        {"time_acceptance", ledger.timeAcceptanceByComponent[component.component]},
        {"expected_events", ledger.expectedEventsByComponent[component.component]},
      });
    }
